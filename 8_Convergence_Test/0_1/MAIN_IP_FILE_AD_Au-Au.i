...
[BCs]
    ...
    [radiation_flux]
        ...
        emissivity_function         = '0.05/(${boltzmann_constant}*4*${T_inf}^3)'  ##https://mooseframework.inl.gov/source/bcs/FunctionRadiativeBC.html
        Tinfinity                   = ${T_inf}
    ...
    [surface_tension] ## Surface tension of Gold https://www.sciencedirect.com/science/article/pii/S0167577X1100440X
        type                    = INSADSurfaceTensionBC
        variable                = 'vel'
        boundary                = 'top'
        include_gradient_terms  = true
        # surface_term_curvature  = surface_term_curvature
        # surface_term_gradient1  = surface_term_gradient1
        # surface_term_gradient2  = surface_term_gradient2
    []
...
[Materials]
   ...
   ## Phase Field Materials 
    [pf_constants]
        ...
        prop_values             = '0.5     ${fparse 0.1/length_scale}     1.5        8.31    0.2'
    []
    ...
    [h1]
        ...
        # # expression              = '(if(eta1 > 1, 1, if(eta1 < 0, 0, eta1)))^3*(6*(if(eta1 > 1, 1, if(eta1 < 0, 0, eta1)))^2 -15*(if(eta1 > 1, 1, if(eta1 < 0, 0, eta1))) + 10)'
   ...
   ## Fluid Flow Materials
    ...
    [surface_tension]  # https://doi.org/10.1016/j.matlet.2011.04.063
        ...
        # expression              = '(4.4 + 0.001*(temp - 1337) + 0.00*(temp - 1337)^2)/(${time_scale}^2)'   ## N/m --> J/m^2 --> kg/s^2 unit
        # expression              = '((1/PHI_Area)*(Vs/(0.196967/rho1))^2 *k_Btz*temp * (0.139*(Es/(R*temp)) - 0.053)) * (1/${time_scale}^2)'
        # outputs                 = 'exodus'
    []
    ...
    [mu_LIQ]
        ...
        coupled_variables       = 'temp'
        expression              = 'exp(-0.1990 + (Q_mu/temp)) / 1000' # The unit is in milli-Pascal-sec so divide by 1000 to convert to Pa.s
        # expression              = 'pseudo_mu1*exp(Q_mu/temp) / 1000' # https://link.springer.com/article/10.1007/s10765-016-2104-7
    []
